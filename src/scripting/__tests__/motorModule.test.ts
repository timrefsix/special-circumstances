...
describe('MotorScriptModule', () => {
  const setup = (initialHeading = 0) => {
    ...
    const module = new MotorScriptModule(world, entity, { initialHeading, durationScale: 0 });
    const engine = new ScriptEngine([module]);
    return { world, entity, module, engine };
  };

  it('moves entities forward relative to heading', async () => {
    const { world, entity, engine } = setup();

    await engine.execute('motor.forward(10)');
  ...
  it('rotates heading to the left and updates position accordingly', async () => {
    const { world, entity, module, engine } = setup();

    await engine.execute('motor.left(90); motor.forward(5)');
  ...
  it('rotates heading to the right and supports backwards movement', async () => {
    const { world, entity, module, engine } = setup();

    await engine.execute('motor.right(45); motor.backwards(10)');
  ...
  it('normalizes headings beyond full rotations', async () => {
    const { module, engine } = setup(10);
    await engine.execute('motor.left(720)');
    expect(module.getHeading()).toBe(10);
    await engine.execute('motor.right(370)');
    expect(module.getHeading()).toBe(0);
  });

  it('throws if the entity has no position component', async () => {
    const { world, entity, engine } = setup();
    world.removeComponent(entity, Position);
    await expect(engine.execute('motor.forward(5)')).rejects.toThrow(ScriptRuntimeError);
  });
});
